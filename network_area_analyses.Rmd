...
v<-read.csv("vivian quanti.csv", h=T, row.names = 1) 
v<-as.matrix(v)
...
athie<-read.csv("w34 athie RC.csv", h=T, row.names = 1) 
...
castro<-read.csv("w44 Castro Cardoso.csv", h=T, row.names = 1) 
...
correia<-read.csv("w30 correia PcAntas.csv", h=T,row.names = 1) 
v<-as.matrix(correia)
df_v<-as.data.frame(as.table(v))
df_v$cod<-paste(df_v$Var1,df_v$Var2,sep = "_")
...
fadini<-read.csv("w36 Fadini MG.csv", h=T,row.names = 1) 
...
fe15<-read.csv("fe15 quanti.csv", h=T, row.names = 1) 
fe15<-as.matrix(fe15)
...
fe25<-read.csv("fe25 quanti.csv", h=T, row.names = 1) 
fe25<-as.matrix(fe25)
...
fe57<-read.csv("fe57 quanti.csv", h=T, row.names = 1) 
fe57<-as.matrix(fe57)
...
GP<-read.csv("w35 Galetti Pizo.csv", h=T, row.names = 1) 
gp<-as.matrix(GP)
...
hasui<-read.csv("w32 Hasui.csv", h=T, row.names = 1) 
...
kaizer<-read.csv("w28 Kaiser original.csv", h=T, row.names = 1) 
...
kindel<-read.csv("w33 Kindel.csv", h=T, row.names = 1) 
...
pizo<-read.csv("pizo quanti.csv", h=T, row.names = 1) 
pizo<-as.matrix(pizo)
...
silva<-read.csv("w37 Wesley.csv", h=T, row.names = 1)# 
...
botelho<-read.csv("w95 Botelho.csv", h=T,row.names = 1)# 
...
rafael<-read.csv("w93 Rafael UFRJ.csv", h=T, row.names = 1)# 
ra<-as.matrix(rafael)
...
nested_p[is.na(nested_p)]<-0
t1 <- as.matrix(t(nested_p))
...
m<-read.csv("test.csv", header = TRUE, row.names=1)
str(m)
which(m[,]>1)

dist<-read.csv("distribution.csv", header = T, row.names = 1)
hist(log(dist$N), xlab = "Number of seed-disperal events (log)", main = "")
...
hist(data$z_wnodf)
## 
m1<-gls(z_wnodf ~ intensity+logarea+forest.type, data=data)
m1a<-update(m1,correlation=corSpher(form=~x+y))
m1b<-update(m1,correlation=corLin(form=~x+y))
m1c<-update(m1,correlation=corRatio(form=~x+y))
m1d<-update(m1,correlation=corGaus(form=~x+y))
...
### second, fit ML for best model selection Zuur, pg 90
m1<-gls(z_wnodf~ intensity+logarea+forest.type, data=data, method="ML")
summary(m1)
dd<-dredge(m1, fixed = "intensity")
print(dd)
summary(get.models(dd,1)[[1]])
...
## finally, refit best models with REML for final results
m1best<-lm(z_wnodf ~ intensity, data=data)#, method="REML")
summary (m1best)
...
z_wnodf<-c(z_wnodf1,z_wnodf2,z_wnodf3,z_wnodf4,z_wnodf5,z_wnodf6,z_wnodf7,z_wnodf8,z_wnodf9,z_wnodf10,z_wnodf11,z_wnodf12,z_wnodf13,z_wnodf14,z_wnodf15,z_wnodf16)
data<-cbind(data,z_wnodf)
plot((data$z_wnodf)~log(data$area_ha))
...
cor.test(c_comm,h2_comm) # 0.12
cor.test(mod_comm,h2_comm) ## 0.80
...
## finally, refit best models with REML for final results
m1best<-lm(interactions ~ logarea+intensity, data=data)#, method="REML")
summary (m1best)
...
V(g)$color <- ifelse(V(g)$name == str_match(V(g)$name, " "), "blue", "orange") #plants
V(g)$color <- ifelse(is.na(V(g)$color), "blue","orange")
